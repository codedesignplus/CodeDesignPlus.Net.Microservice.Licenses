...
) : DomainEvent(aggregateId, eventId, occurredAt, metadata)
{
    ...
    public static LicenseUpdatedDomainEvent Create(Guid aggregateId, string name, string shortDescription, string description, List<ModuleEntity> modules, List<Price> prices, string icon, string TermsOfService, Dictionary<string, string> attributes, bool isActive, bool isPopular)
    {
        return new LicenseUpdatedDomainEvent(aggregateId, name, shortDescription, description, modules, prices, icon, TermsOfService, attributes, isActive, isPopular);
    }
}
