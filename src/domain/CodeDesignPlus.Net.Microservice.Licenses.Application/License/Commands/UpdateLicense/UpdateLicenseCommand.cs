...
[DtoGenerator]
public record UpdateLicenseCommand(Guid Id, string Name, string ShortDescription, string Description, List<ModuleDto> Modules, List<Price> Prices, string Icon, string TermsOfService, Dictionary<string, string> Attributes, bool IsActive, bool IsPopular) : IRequest;
...
public class Validator : AbstractValidator<UpdateLicenseCommand>
{
    public Validator()
    {
        ...
        RuleFor(x => x.Icon).NotEmpty().NotNull();
